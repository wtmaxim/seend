...
export const metadata: Metadata = { title: "Document partagé · Seend" }

function LinkUnavailable() {
  return (
    <main className="flex min-h-svh items-center justify-center p-6 text-center">
      <div>
        <p className="text-sm font-medium text-foreground">Ce lien n&apos;est plus disponible.</p>
        <p className="mt-1 text-xs text-muted-foreground">Il a peut-être expiré ou a été révoqué.</p>
      </div>
    </main>
  )
}
          ...
          <div className="overflow-hidden rounded-2xl border border-border">
            {shareLink.dataroom.documents.length === 0 ? (
              <p className="p-10 text-center text-xs text-muted-foreground">
                Cette dataroom ne contient aucun document.
              </p>
            ) : (
      ...
      <div className="mx-auto max-w-4xl space-y-4">
        ...
        {renderError ? (
          <div className="rounded-2xl border border-border p-10 text-center">
            <p className="text-sm text-foreground">Ce document ne peut pas être affiché.</p>
            <p className="mt-1 text-xs text-muted-foreground">Contactez la personne qui vous a envoyé ce lien.</p>
          </div>
        ) : (
          <DocumentViewer
            token={token}
